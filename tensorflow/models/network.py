...
class Network(object):
    ...
    def __init__(self, inputs, batch, rate, is_training, trainable = True):
    ...
    def make_var(self, name, shape):
        ...
        return tf.compat.v1.get_variable(name, shape, dtype = 'float32', trainable=self.trainable, use_resource=False)
    ...
    @layer
    def dropout(self, input_data, rate, name):
        return tf.nn.dropout(input_data, rate=float(rate), name=name)
